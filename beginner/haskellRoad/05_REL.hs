...
-}
{-
    Relation        A relation, R, is a set of 'ordered pairs'
    Domain          the set of all the first coordinates of pairs in R
    Range           the set of all the second coordinates of pairs in R
        
    Example:
        If A and B are sets, then A x B is a relation.
        The 'domain' of the relation (AxB) is A IF B is non-empty
        otherwise Ax0 = 0, domain (Ax0) = 0, range (Ax0) = 0 where
        '0' is the Empty Set.
        
    The relation R is a relation 'from A to B' or 'between A and B'
    A relation from A to A is called 'on A'
    If R is a relation on A, then A is called the 'underlying set'
    (of the structure that consists of the domain A and the relation R)
    
    Example:
        R = {(1,4),(1,5),(2,5)} is a relation from {1,2,3} to {4,5,6}
        and R is also a relation 'on' {1,2,4,5,6} while the 'domain'
        of R is {1,2} and the 'range' of R is {4,5}
        
        [Note: No idea why '3' and '6' are included in the 'from/to']
-}
import Data.List
import SetOrd
