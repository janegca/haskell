...
{- Exercise 7
   ...
   Add datatypes and parsers to support Scheme's full numeric tower
   (number, complex, real, rational. and integer).
   
   Haskell has built-in types to represent many of these; check the 
   Prelude. For the others, you can define compound types that represent
   eg. a Rational as a numerator and denominator, or a Complex as a real 
   and imaginary part (each itself a Real).
...
data LispVal = Atom        String
             ...
             | Complex    (Complex Double)  -- added for Exercise 7
             | Ratio       Rational         
...
parseComplex = do real <- ( try parseFloat <|> parseDefaultDigit)
                  char '+'
                  img <- ( try parseFloat <|> parseDefaultDigit)
                  char 'i'
